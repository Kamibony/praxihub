...
export default function MatchmakingPage() {
  ...
  const getScoreColor = (score: number) => {
    ...
    return "text-theme-muted bg-slate-800/50 border-slate-700/50";
  };

  return (
    <div className="min-h-screen dark bg-slate-900 p-8 font-sans text-theme-primary">
      <div className="max-w-4xl mx-auto">
        <header className="mb-8 flex justify-between items-center">
          <div>
            <Link href="/student/dashboard" className="text-sm text-theme-muted hover:text-theme-primary mb-2 inline-block transition">
              ← Zpět na dashboard
            </Link>
            <h1 className="text-3xl font-bold text-white flex items-center gap-2">✨ AI Matchmaking</h1>
            <p className="text-theme-muted mt-1">Hledáme pro tebe ideální firmu na základě tvých dovedností.</p>
          </div>
        </header>
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-lg text-theme-secondary font-medium">✨ Gemini AI analyzuje tvůj profil a porovnává ho s firmami...</p>
          </div>
        ) : error ? (
          <div className="bg-slate-800/75 backdrop-blur-md p-8 rounded-3xl shadow-lg border border-theme-border text-center">
          ...
          <div className="bg-slate-800/75 backdrop-blur-md p-8 rounded-3xl shadow-lg border border-theme-border text-center">
             <p className="text-theme-muted">Nenašli jsme žádnou shodu. Zkus přidat více dovedností na svém profilu.</p>
          </div>
        ) : (
          <div className="grid gap-6">
            {matches.map((match) => (
              <div key={match.organizationId} className="bg-slate-800/75 backdrop-blur-md p-6 rounded-3xl shadow-sm border border-theme-border hover:border-indigo-500/50 transition relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start gap-4 relative z-10">
                  <div>
                    ...
                    <p className="text-sm text-theme-muted mb-2">{match.companyEmail}</p>

                    <div className="flex flex-wrap gap-2 mb-3">
                      {match.lookingFor && match.lookingFor.map((tag, i) => (
                        <span key={i} className="px-2 py-0.5 bg-theme-panel text-theme-secondary text-xs rounded-full border border-slate-700/50">
                          {tag}
                        </span>
                      ))}
                    </div>
