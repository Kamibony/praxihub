...
async function createCertificatePdf(snapshotData, snapshotId) {
    ...
    page.drawText(`Student: ${snapshotData.studentName}`, { x: 50, y: height - 160, size: 14, font: customFont });
    page.drawText(`Společnost: ${snapshotData.companyName || snapshotData.organization_name || 'Neznámá'}`, { x: 50, y: height - 190, size: 14, font: customFont });
    if (snapshotData.start_date && snapshotData.end_date) {
        page.drawText(`Termín: ${snapshotData.start_date} - ${snapshotData.end_date}`, { x: 50, y: height - 220, size: 14, font: customFont });
    }

    page.drawText(`Hodnocení AI (MŠMT KRAU): Úspěšně splněno`, { x: 50, y: height - 260, size: 14, font: customFont, color: rgb(0, 0.5, 0) });
