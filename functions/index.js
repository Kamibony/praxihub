...
exports.analyzeContract = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    // Trigger only if status changed to ANALYZING
    if (newData.status === "ANALYZING" && previousData.status !== "ANALYZING") {
      ...
      try {
        ...
        if (!fileUrl) throw new Error("Missing contract_url");
        ...
        const prompt = `
          Analyze this internship contract.
        ...
        await change.after.ref.update({
          organization_name: extractedData.organization_name || "Unknown Company",
