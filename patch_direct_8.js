...
const regex = /<td className="px-6 py-4 whitespace-nowrap">\s*<span\s*className=\{`px-2\.5 py-0\.5 inline-flex text-xs leading-5 font-semibold rounded-full border [\s\S]*?<\/span>\s*<\/td>\s*<td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">/;
...
if (regex.test(content)) {
    ...
    content = content.replace(
      /\{item\.status === "PENDING_ORG_APPROVAL" \? \([\s\S]*?Detail\n\s*<\/button>\n\s*<\/>\n\s*\)}/,
      `{item.status === "PENDING_COORDINATOR" && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
